    ...
    execute: function () {
      ...
      _export("Main", Main = (_dec = ccclass('Main'), _dec2 = property({
        ...
        createColorArray() {
          ...
          const yPercent = Number(this.editBoxY.getComponent(EditBox).string); // 初始化矩阵
        ...
        drawMatrix() {
          this.nodeParent.removeAllChildren(); //从左到右 从上到下

          for (let i = 0; i < 10; i++) {
            for (let j = 0; j < 10; j++) {
              const curColor = this.matrix[i][j];
              const xx = 0 + 60 * i;
              const yy = 0 - 60 * j;
        ...
        getProbabilities(m, n, x, y) {
          ...
          const probabilities = this.colors.map(() => baseProb); // 获取上方和左方颜色  

          let topColor = m > 0 ? this.matrix[m - 1][n] : null;
          let leftColor = n > 0 ? this.matrix[m][n - 1] : null; // 增加上方和左方颜色的概率  
...
//# sourceMappingURL=d6a79a68cc55935100f88da13764138acca5748d.js.map